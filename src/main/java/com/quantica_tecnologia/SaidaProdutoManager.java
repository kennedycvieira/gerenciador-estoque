...
public class SaidaProdutoManager {
    ...
    public static boolean registrarSaidaProduto(String usuario, int produtoId, int portadorId, int destinoId, double quantidade) {
        Connection conn = null;
        try {
            conn = DatabaseManager.connect();
            conn.setAutoCommit(false);
            
            // First verify if the product exists and get current quantity
    ...
    public static List<SaidaProdutoInfo> listarSaidasCompletas() {
        List<SaidaProdutoInfo> saidas = new ArrayList<>();
        
        try (Connection conn = DatabaseManager.connect();
             PreparedStatement pstmt = conn.prepareStatement(
                     "SELECT s.id, s.produto_id, p.identificador, p.nome, s.quantidade, p.unidade, " +
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    SaidaProdutoInfo saida = new SaidaProdutoInfo(
                        rs.getInt("id"),
    ...
    public static class SaidaProdutoInfo {
        ...
        public SaidaProdutoInfo(int id, int produtoId, String produtoIdentificador, String produtoNome, 
                               double quantidade, String unidade, int portadorId, String portadorNome, 
                               int destinoId, String destinoNome, Date dataSaida) {
            this.id = id;
            this.produtoId = produtoId;
            this.produtoIdentificador = produtoIdentificador;
